...
class Vector3 extends Array {
  ...
  project(camera: Camera, helperMatrix: Matrix4): Vector3 {
    const m = helperMatrix.copy(camera.matrixWorld).invert()
    return this.applyMatrix4(m).applyMatrix4(camera.projectionMatrix);
  }

  unproject(camera: Camera, helperMatrix: Matrix4): Vector3 {
    const m = helperMatrix.copy(camera.projectionMatrix).invert()
    return this.applyMatrix4(m).applyMatrix4(camera.matrixWorld);
  }
  ...
  setFromMatrixScale(m: Matrix4): Vector3 {
    const sx = this.setFromMatrixColumn(m, 0).len();
    const sy = this.setFromMatrixColumn(m, 1).len();
    const sz = this.setFromMatrixColumn(m, 2).len();
